...
創角先掃 `catalog/`，需要閱讀具體法術內容時再按 spell id / 名稱進 `index/`。一般查詢法術時可以搜尋完整 index，並回報來源；source scope 主要限制：
...
`catalog/A.jsonl` … `catalog/Z.jsonl` 是創角／廣搜用的輕量索引；`index/A.jsonl` … `index/Z.jsonl` 是完整法術條目。完整 index 每列一個法術，主要欄位：
