...
最直接的 `[D100_CANON]` 範例是吸血鬼（該隱一族）的 `魔眼支配`：透過凝視擊垮對手意志，目標必須進行**抗控制**檢定，失敗後落入吸血鬼控制。
...
神器級詞綴 `聖輝`／`邪典` 等也會在自己的條文中**明確指定**某些情況要求抗魔法檢定，支持「抗魔法是否出場應由能力規則決定」而非所有魔法自動附加一次抗魔法骰 `[D100_DERIVED]`。
